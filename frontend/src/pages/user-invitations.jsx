      ...
      <div key={index} className="py-1 cursor-pointer">
        <Card>
          <CardBody className="flex">
            <div
              onClick={() => onClickViewEvent(e.event.pk)}
              className="flex-grow"
            >
              <Typography className="font-semibold mb-1" variant="h5">
                {e.event.title}
              </Typography>
              {moment(e.event.date_scheduled).format("M/D/yyyy")} at{" "}
              {moment(e.event.time_scheduled, "HH:mm:ss").format("hh:mm A")}
              <div className="flex flex-row items-center mt-1">
                <UserAvatar user={e.event.host} className=" " />
                <Typography className="ml-1"> {e.host}</Typography>
              </div>
            </div>
            <div className=" self-center">
              <RSVP
                ...
                response={e.response === null ? "null" : e.response.toString()}
              />
            </div>
          </CardBody>
        </Card>
